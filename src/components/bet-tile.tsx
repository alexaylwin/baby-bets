...
export const BetTile = (props: {
  ...
  const betPercentages = pool?.betsPerOption.map((bo, index) => {
    return {
      opt: bo.option,
      percent: ((bo.amount / pool.totalPool) * 100).toFixed(2),
    };
  });
  ...
  return (
    <div className="grid grid-rows-1 place-items-center px-4 w-11/12">
      <h2 className="font-bold text-white py-6 text-2xl">{bet.description}</h2>
      ...
      <div className="grid grid-cols-2 place-items-center w-full mt-10 ">
        <div className="flex flex-wrap items-stretch w-28 relative">
          <div className="flex -mr-px">
            <span className="font-bold text-white bg-gray-700 -tracking-normal flex items-center leading-normal rounded rounded-r-none border border-r-0 border-gray-100 px-3 whitespace-no-wrap text-sm">
              $
            </span>
          </div>
          <input
            type="number"
            className="bg-purple-400 flex-shrink flex-grow flex-auto leading-normal w-px border h-10 border-gray-100 text-gray-100 placeholder-gray-100/50 rounded rounded-l-none px-3 relative focus:border-blue focus:shadow"
            placeholder="Amount"
            onChange={(e) => {
        ...
        <span
          className="w-42
        border border-solid border-gray-100 rounded 
        px-3 py-1.5 text-gray-100 font-bold"
        >
          Est. Payout: $
          {calcEstimatedPayout(
            selectedAmount,
